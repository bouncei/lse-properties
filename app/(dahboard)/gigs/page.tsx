...
import CreateGigDialog from "@/components/dialogs/create-gig";

const GigsPage = () => {
  const { gigs } = useGigsStore();
  const [createModal, setCreateModdal] = useState<boolean>(false);
    ...
    <>
      <div className="flex flex-col  items-start gap-8 text-sm ">
        ...
        <div className="flex items-center justify-between w-full">
          ...
          {/* CREATE GIG */}
          <Button
            className="bg-green-700 hover:bg-none hover:opacity-70 hover:bg-green-600"
            onClick={() => setCreateModdal(true)}
          >
      ...
      <CreateGigDialog
        isOpen={createModal}
        onClose={() => setCreateModdal(!createModal)}
      />
    </>
  );
};
